...
class Application:
    ...
    def __init__(self):
        ...
        # Adjusted width to fit a 13-inch laptop
        self.root.geometry("1200x700")

        self.panel = tk.Label(self.root)
        # Adjusted position and size
        self.panel.place(x=60, y=60, width=480, height=480)

        self.panel2 = tk.Label(self.root)
        # Adjusted position and size
        self.panel2.place(x=640, y=60, width=400, height=400)

        self.T = tk.Label(self.root)
        self.T.place(x=400, y=10)  # Adjusted position
        self.T.config(text="Sign Language To Text Conversion",
                      font=("Courier", 20, "bold"))

        self.panel3 = tk.Label(self.root)
        self.panel3.place(x=260, y=560)  # Adjusted position

        self.T1 = tk.Label(self.root)
        self.T1.place(x=10, y=550)  # Adjusted position
        self.T1.config(text="Character :", font=("Courier", 20, "bold"))

        self.panel5 = tk.Label(self.root)
        self.panel5.place(x=240, y=620)  # Adjusted position

        self.T3 = tk.Label(self.root)
        self.T3.place(x=10, y=620)  # Adjusted position
        self.T3.config(text="Sentence :", font=("Courier", 20, "bold"))

        self.T4 = tk.Label(self.root)
        self.T4.place(x=10, y=680)  # Adjusted position
        self.T4.config(text="Suggestions :", fg="red",
                       font=("Courier", 20, "bold"))

        self.b1 = tk.Button(self.root)
        self.b1.place(x=300, y=680)  # Adjusted position

        self.b2 = tk.Button(self.root)
        self.b2.place(x=500, y=680)  # Adjusted position

        self.b3 = tk.Button(self.root)
        self.b3.place(x=700, y=680)  # Adjusted position

        self.b4 = tk.Button(self.root)
        self.b4.place(x=900, y=680)  # Adjusted position

        self.speak = tk.Button(self.root)
        self.speak.place(x=1120, y=610)  # Adjusted position
        self.speak.config(text="Speak", font=("Courier", 15),
                          wraplength=100, command=self.speak_fun)

        self.clear = tk.Button(self.root)
        self.clear.place(x=1030, y=610)  # Adjusted position
        self.clear.config(text="Clear", font=("Courier", 15),
    ...
    def video_loop(self):
        try:
            ...
            hands, cv2image = hd.findHands(frame, draw=False)
            ...
            if hands:
                ...
                hand = hands[0]
                x, y, w, h = hand['bbox']
                image = cv2image_copy[y:y+h, x:x+w]

                white = cv2.imread(
                    "./white.jpg")
                # img_final=img_final1=img_final2=0

                handz,img_s= hd2.findHands(image, draw=True, flipType=True)
                print(" ", self.ccc)
                self.ccc += 1
                if handz:
                    hand = handz[0]
                    ...
                    for i in range(21):
                        cv2.circle(
                            white, (self.pts[i][0] + os, self.pts[i][1] + os1), 2, (0, 0, 255), 1)
                    ...
                    self.b1.config(text=self.word1, font=(
                        "Courier", 20), wraplength=825, command=self.action1)
                    self.b2.config(text=self.word2, font=(
                        "Courier", 20), wraplength=825,  command=self.action2)
                    self.b3.config(text=self.word3, font=(
                        "Courier", 20), wraplength=825,  command=self.action3)
                    self.b4.config(text=self.word4, font=(
                        "Courier", 20), wraplength=825,  command=self.action4)

            self.panel5.config(text=self.str, font=(
                "Courier", 30), wraplength=1025)
        except Exception:
    ...
    def predict(self, test_image):
        ...
        l = [[5, 2], [5, 3], [3, 5], [3, 6], [3, 0], [3, 2], [6, 4], [6, 1], [6, 2], [6, 6], [6, 7], [6, 0], [6, 5],
             [4, 1], [1, 0], [1, 1], [6, 3], [1, 6], [5, 6], [5, 1], [
                 4, 5], [1, 4], [1, 5], [2, 0], [2, 6], [4, 6],
        ...
        # con for [i][pqz]
        l = [[5, 4], [5, 5], [5, 1], [0, 3], [0, 7], [5, 0],
             [0, 2], [6, 2], [7, 5], [7, 1], [7, 6], [7, 7]]
        pl = [ch1, ch2]
        if pl in l:
            if ((self.pts[6][1] < self.pts[8][1] and self.pts[10][1] < self.pts[12][1] and self.pts[14][1] < self.pts[16][1] and
                 self.pts[18][1] > self.pts[20][1])):
